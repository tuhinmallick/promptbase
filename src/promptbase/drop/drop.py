...
def solve(idx):
    ...
    for _ in range(5):
        ...
        if CHAT_MODE:
            prompts[idx].append({"role": "assistant", "content": response['response']["choices"][0]["text"]})
            prompts[idx].append({"role": "user", "content": "----\n**Final Answer**: "})
        else:
            prompts[idx] += response['response']["choices"][0]["text"]
            prompts[idx] += "----\n**Final Answer**: "

        response_final = text_completion(prompt=prompts[idx], model=model_name, max_tokens=100, log_file=answer_file_name, max_trial=5, temperature = 0, stop=["**", '--', '\n'])
...
def check_span(response, answer_spans):
    ...
    response = ''.join(
        c for c in response.lower() if c not in string.punctuation)

    response_words = set(response.split())

    for answer_span in answer_spans:
        answer_span = ''.join(c for c in answer_span.lower()
...
def check_number(response, answer_number):
    ...
    return any(float(num.replace(',', '')) == float(answer_number.replace(',', '')) for num in response_numbers)
